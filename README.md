...
- **Клапан 2 (`valve_spring_to_tank`)**: Отворен, ако `tank_high_level` е false (резервоарът не е пълен). Затворен, ако `tank_high_level` е true (резервоарът е пълен).
- **Клапан 6 (`valve_tank_to_pump`)**: Отворен, за да подаде вода от резервоара към хидрофорната помпа.
- **Клапан 7 (`valve_pump_to_building`)**: Отворен, за да подаде вода от помпата към сградата.
...
- **Клапан 4 (`valve_supply_to_pump`)**: Затворен, за да не подава вода от водопровода директно към помпата.
- **Клапан 5 (`valve_spring_to_pump`)**: Затворен, защото в този режим помпата черпи вода от резервоара, а не директно от сондажа.
...
- **Клапан 2 (`valve_spring_to_tank`)**: Отворен, за да продължи пълненето на резервоара от сондажа.
- **Клапан 4 (`valve_supply_to_pump`)**: Отворен, за да подаде вода от водопровода към хидрофорната помпа.
- **Клапан 7 (`valve_pump_to_building`)**: Отворен, за да подаде вода от помпата към сградата.

### Неактивни клапани:

- **Клапан 1 (`valve_supply_to_tank`)**: Затворен, защото резервоарът не се пълни от водопровода.
- **Клапан 3 (`valve_pump_to_tank`)**: Затворен, за да не връща вода от помпата обратно към резервоара.
- **Клапан 5 (`valve_spring_to_pump`)**: Затворен, защото водоподаването се извършва от водопровода, а не от сондажа директно към помпата.
...
- **Клапан 4 (`valve_supply_to_pump`)**: Отворен, за да подаде вода от водопровода към хидрофорната помпа.
- **Клапан 5 (`valve_spring_to_pump`)**: Отворен, за да подаде вода от сондажа директно към помпата.
- **Клапан 7 (`valve_pump_to_building`)**: Отворен, за да подаде вода към сградата.
...
- **Клапан 4 (`valve_supply_to_pump`)**: Отворен, за да подаде вода от водопровода към хидрофорната помпа. Водата от водопровода минава през помпата.
- **Клапан 5 (`valve_spring_to_pump`)**: Отворен, за да подаде вода от сондажа директно към помпата. Водата от сондажа също се движи през помпата.
- **Клапан 3 (`valve_pump_to_tank`)**: Отворен, за да върне вода от помпата в резервоара. Помпата връща водата обратно в резервоара след смучене от водопровода и сондажа.

### Неактивни клапани:

- **Клапан 1 (`valve_supply_to_tank`)**: Затворен, защото водопроводът подава вода към помпата, а не директно към резервоара.
- **Клапан 2 (`valve_spring_to_tank`)**: Затворен, защото водата от сондажа се движи към помпата, а не директно към резервоара.
- **Клапан 6 (`valve_tank_to_pump`)**: Затворен, защото резервоарът се пълни, а не подава вода към помпата.
- **Клапан 7 (`valve_pump_to_building`)**: Затворен, защото няма активна консумация в сградата в този режим, водата се връща в резервоара.
